...
const mergeStringArray = (current = [], incoming = [], maxLength) =>
  normalizeStringArray([...(Array.isArray(current) ? current : []), ...incoming], maxLength);
...
const normalizeDetailLevel = (value) => {
  ...
  return ['brief', 'balanced', 'detailed'].includes(normalized) ? normalized : '';
};
...
const toPlainMemory = (memory = {}) => {
  ...
  return {
    ...
    favoriteTopics: normalizeStringArray(memory.favoriteTopics, FIELD_LIMITS.topic),
    recurringGoals: normalizeStringArray(memory.recurringGoals, FIELD_LIMITS.goal),
    writingStyle: {
    ...
    taskPriorities: normalizeStringArray(memory.taskPriorities, FIELD_LIMITS.priority),
...
const getAssistantMemoryForUser = async (userId) => {
  const user = await User.findById(userId).select('preferences timezone firstName lastName');
  if (!user) {
...
const updateAssistantMemoryForUser = async (userId, args = {}) => {
  ...
  const mergeMode =
    normalizeText(args?.mergeMode, 20).toLowerCase() === 'replace' ? 'replace' : 'merge';
  const current = toPlainMemory(user.preferences?.assistantMemory);
  const next = toPlainMemory(current);

  const clearFields = normalizeStringArray(args?.clearFields, 60).filter((field) =>
    ...
    next.writingStyle =
      mergeMode === 'replace'
        ...
        : {
            ...next.writingStyle,
            ...Object.fromEntries(
              Object.entries(writingStyle).filter(([, value]) => Boolean(value))
            )
          };
  }
...
const formatAssistantMemoryForPrompt = async (userId) => {
  ...
  if (
    ...
    lines.push(
      `- Working hours: ${[
        ...
        working.startTime && working.endTime ? `${working.startTime}-${working.endTime}` : '',
  ...
  if (memory.preferredMeetingLengthMinutes) {
    lines.push(`- Preferred meeting length: ${memory.preferredMeetingLengthMinutes} minutes`);
  }
  ...
  if (style.tone || style.detailLevel || style.format || style.notes) {
    lines.push(
      `- Writing style: ${[style.tone, style.detailLevel, style.format, style.notes]
        .filter(Boolean)
        .join('; ')}`
    );
  }
  ...
  if (memory.additionalPreferences.length) {
    lines.push(`- Additional preferences: ${memory.additionalPreferences.join('; ')}`);
  }
